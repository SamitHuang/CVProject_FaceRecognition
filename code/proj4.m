...
POS_FILE = 'features_pos_3.mat';
NEG_FILE = 'features_neg_3.mat';
if exist(POS_FILE,'file') 
    load(POS_FILE,'features_pos');
else
    features_pos = get_positive_features( train_path_pos, feature_params );
end
 
if exist(NEG_FILE, 'file')
    load(NEG_FILE,'features_neg')
else
...
MINE_W_FILE = 'mine_w_3.mat';
MINE_B_FILE = 'mine_b_3.mat';

if MINE_FLAG ==1
    if exist(MINE_W_FILE, 'file') && exist(MINE_B_FILE, 'file')
        load(MINE_W_FILE, 'mine_w');
        load(MINE_B_FILE, 'mine_b');
    else
        [mine_w, mine_b] = mine_hard_negative(features_pos, features_neg, non_face_scn_path, w, b, feature_params);
    end
end
